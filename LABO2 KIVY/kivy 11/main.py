...
class Game(Widget):
    ...
    def cambiarPista(self):
        global estadoPista
        if self.color1==[.5,.5,.5,.5] and estadoPista==0:
            self.color1=[1,0,0,1]
            self.color2=[0,1,0,1]
            self.color3=[0,0,1,1]
            self.pista_button.text = "Pista carretera"
            estadoPista=1
            return
        if self.color1==[1,0,0,1] and estadoPista==1:
            self.color1=[.5,.5,.5,.5]
            self.color2=[.75,.75,.75,.75]
            self.color3=[.5,.5,.5,.5]
            estadoPista=0
            self.pista_button.text = "Pista de colores"
            return

    def volverPista(self):
        self.color1=[.5,.5,.5,.5]
        self.color2=[.75,.75,.75,.75]
        self.color3=[.5,.5,.5,.5]
    ...
    def mostrar_menu(self):
        self.start_button.pos = (self.width / 2 - tamanio_boton_x/2, self.height/2+self.start_button.height)
        self.color_button.pos = (self.width / 2 - tamanio_boton_x/2, self.height/2)
        self.pista_button.pos = (self.width / 2 - tamanio_boton_x/2, self.height/2-tamanio_boton_y)
        self.quit_button.pos = (self.width / 2 - tamanio_boton_x/2, self.height/2-tamanio_boton_y*2)
    ...
    def update(self, dt):
        ...
        #print("Velocidad enemigo actual: "+ str(self.car.velocity_y))
        #print("Aumentar Nivel: "+ str(self.aumentarNivel))
        
        if (self.car.x == self.player.x):        
            if ( 50 < self.car.y <= 235):
                global vivo
                vivo = False
        if (not vivo):
...
class GameApp(App):
    def build(self):
        ...
        game.pista_button = game.ids.pistaColor_button1
